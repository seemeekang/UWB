...
def compute_rq_tag(og_anchor_coordinates, og_anchor_distance_matrix, og_tag_coordinates, dmin):
    robust_nodes, cal_anchor_coordinates, non_trilaterated_nodes, robust_tris, sort_loc_best = compute_RQ_algo2(
        og_anchor_distance_matrix, dmin)
    MSE_dist_anchor = calculate_dist_MSE_redundant(og_anchor_coordinates, sort_loc_best)
    MSE_loc_anchor = calculate_loc_MSE_redundant(og_anchor_coordinates, sort_loc_best)
...
def main():
    ...
    cal_a_crdnt, MSE_dist_a, MSE_loc_a, CSR, cal_t_crdnt, MSE_loc_t = compute_rq_tag(
            og_anchor_coordinates, og_anchor_distance_matrix, og_tag_coordinates, dmin)
    

    n_cal_a_crdnt, n_MSE_dist_a, n_MSE_loc_a, n_CSR, n_cal_t_crdnt, n_MSE_loc_t = compute_rq_tag(
            og_anchor_coordinates, n_anchor_distance_matrix, og_tag_coordinates, dmin)
